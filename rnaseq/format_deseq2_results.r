format_deseq2_results <- function(dds, res, sample_names, groups) {
  ...
  combined_df$falsePos <- seq_len(nrow(combined_df)) * combined_df$FDR

  # Identify sample names for each experimental group
  sample_names_A <- sample_names[colData(dds)$group == groups[[1]]]
  sample_names_B <- sample_names[colData(dds)$group == groups[[2]]]
